...
if userQuery := st.chat_input("What is your question?"):
    ...
    with st.chat_message("assistant"):
        ...
        with st.status("Answering!", expanded=True) as status:
            response = logic_lab_3.converse_with_bedrock(list(st.session_state.messages.values()))
            answer, tool, parameters_used = logic_lab_3.parse_response(response)
